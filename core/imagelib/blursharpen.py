...
def LinearMotionBlur(image, size, angle):
    ci_size = int(size)
    k = np.zeros((ci_size, ci_size), dtype=np.float32)
    k[ (ci_size-1)// 2 , :] = np.ones(ci_size, dtype=np.float32)
    k = cv2.warpAffine(k, cv2.getRotationMatrix2D( (ci_size / 2 -0.5 , ci_size / 2 -0.5 ) , angle, 1.0), (ci_size, ci_size) )
    k = k * ( 1.0 / np.sum(k) )
