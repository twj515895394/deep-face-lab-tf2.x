...
def gen_warp_params (w, flip=False, rotation_range=[-10,10], scale_range=[-0.5, 0.5], tx_range=[-0.05, 0.05], ty_range=[-0.05, 0.05], rnd_state=None, warp_rnd_state=None  ):
    ...
    #random warp V1
    cell_size = [ ci_w // (2**i) for i in range(1,4) ] [ warp_rnd_state.randint(3) ]
    cell_count = ci_w // cell_size + 1
    grid_points = np.linspace( 0, ci_w, cell_count)
    ...
    mapx = cv2.resize(mapx, (ci_w+cell_size,)*2 )[half_cell_size:-half_cell_size,half_cell_size:-half_cell_size].astype(np.float32)
    mapy = cv2.resize(mapy, (ci_w+cell_size,)*2 )[half_cell_size:-half_cell_size,half_cell_size:-half_cell_size].astype(np.float32)
    ##############
    ...
    #random transform
    random_transform_mat = cv2.getRotationMatrix2D((ci_w // 2, ci_w // 2), rotation, scale)
    random_transform_mat[:, 2] += (tx*ci_w, ty*ci_w)
    ...
    params['umat'] = u_mat
    params['w'] = ci_w
    params['rw'] = int(rw) if rw is not None else None
    params['flip'] = p_flip
