...
class InteractDesktop(InteractBase):
    ...
    def on_create_window (self, wnd_name):
        try:
            ...
            global _HEADLESS_MODE, _headless_preview_dir
            if 'not implemented' in str(e).lower() or 'gtk' in str(e).lower():
                if not _HEADLESS_MODE:
                    _HEADLESS_MODE = True
                    _headless_preview_dir = os.path.join(os.getcwd(), '_preview')
                    os.makedirs(_headless_preview_dir, exist_ok=True)
                    print(f"[Headless Mode] No GUI display available. Previews will be saved to: {_headless_preview_dir}")
            else:
                raise
