...
class Conv2D(nn.LayerBase):
    """
    Optimized Conv2D layer for TensorFlow 2.21.0 and Blackwell architecture
    
    Features:
    - Blackwell Tensor Core optimization (NCHW format preferred)
    - bfloat16 mixed precision support
    - Equalized learning rate (from ProGAN)
    ...
    def __init__(self, in_ch, out_ch, kernel_size, strides=1, padding='SAME', dilations=1, use_bias=True, use_wscale=False, kernel_initializer=None, bias_initializer=None, trainable=True, dtype=None, **kwargs ):
        ...
        if dtype is None:
            if nn.is_blackwell() and nn.floatx == tf.float32:
                dtype = tf.float16
            else:
                dtype = nn.floatx
        ...
        self._is_optimized_for_blackwell = nn.is_blackwell()
    ...
    def forward(self, x):
        ...
        if self.dtype != original_dtype and self._is_optimized_for_blackwell:
    ...
    def __str__(self):
        ...
        if self._is_optimized_for_blackwell:
            r += "[Blackwell-optimized] "
        return r
