...
class PatchDiscriminator(nn.ModelBase):
    def on_build(self, patch_size, in_ch, base_ch=None, conv_kernel_initializer=None, dtype=None):
        ...
        for i, (kernel_size, strides) in enumerate(kernels_strides):
            ...
            self.convs.append ( nn.Conv2D( prev_ch, cur_ch, kernel_size=kernel_size, strides=strides, padding='SAME', kernel_initializer=conv_kernel_initializer, dtype=dtype) )
            prev_ch = cur_ch

        self.out_conv =  nn.Conv2D( prev_ch, 1, kernel_size=1, padding='VALID', kernel_initializer=conv_kernel_initializer, dtype=dtype)
...
class UNetPatchDiscriminator(nn.ModelBase):
    ...
    def on_build(self, patch_size, in_ch, base_ch = 16, use_fp16 = False, use_bf16 = False):
        self.use_fp16 = use_fp16
        self.use_bf16 = use_bf16
        # 确定卷积层的dtype
        if use_bf16:
            conv_dtype = tf.bfloat16
        elif use_fp16:
            conv_dtype = tf.float16
        else:
            conv_dtype = tf.float32 
