...
def style_loss(target, style, gaussian_blur_radius=0.0, loss_weight=1.0, step_size=1):
    def sd(content, style, loss_weight):
        ...
        return (mean_loss + std_loss) * ( tf.cast(loss_weight, content.dtype) / tf.cast(content_nc.value, content.dtype) )
...
def dssim(img1,img2, max_val, filter_size=11, filter_sigma=1.5, k1=0.01, k2=0.03):
    ...
    # 确保卷积核的类型与输入张量的类型匹配
    kernel = tf.cast(kernel, img1.dtype)
    ...
    dssim = tf.cast(dssim, tf.float32)
    return dssim
...
def rgb_to_lab(srgb):
    ...
    # 确保mask类型与输入类型匹配
    linear_mask = tf.cast(srgb_pixels <= 0.04045, dtype=srgb.dtype)
    exponential_mask = tf.cast(srgb_pixels > 0.04045, dtype=srgb.dtype)
    ...
    xyz_normalized_pixels = tf.multiply(xyz_pixels, tf.cast([1/0.950456, 1.0, 1/1.088754], dtype=srgb.dtype))

    epsilon = 6/29
    linear_mask = tf.cast(xyz_normalized_pixels <= (epsilon**3), dtype=srgb.dtype)
    exponential_mask = tf.cast(xyz_normalized_pixels > (epsilon**3), dtype=srgb.dtype)
    ...
    lab_pixels = tf.matmul(fxfyfz_pixels, fxfyfz_to_lab) + tf.cast([-16.0, 0.0, 0.0], dtype=srgb.dtype)
...
def bilinear_sampler(img, x, y):
    ...
    # recast as same type as input for delta calculation
