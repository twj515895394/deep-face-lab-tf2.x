...
class RMSprop(nn.OptimizerBase):
    ...
    def get_update_op(self, grads_vars):
        ...
        for i, (g,v) in enumerate(grads_vars):
            ...
            # 处理bfloat16类型
            try:
                resolution = np.finfo(g.dtype.as_numpy_dtype).resolution
            except ValueError:
                # 对于bfloat16类型，使用固定的分辨率值
                resolution = 1.1920929e-07  # bfloat16的分辨率
            v_diff = - lr * g / (tf.sqrt(new_a) + resolution  )
            if self.lr_dropout != 1.0:
