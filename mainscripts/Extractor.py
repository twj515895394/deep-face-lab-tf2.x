...
class ExtractSubprocessor(Subprocessor):
    ...
    @staticmethod
    def get_devices_for_config (type, device_config, workers_per_gpu=1):
           ...
           'all'       in type:
            ...
            if not cpu_only:
                ...
                for device in devices:
                    count = 1 if type == 'landmarks-manual' else workers_per_gpu
    ...
    def __init__(self, input_data, type, image_size=None, jpeg_quality=None, face_type=None, output_debug_path=None, manual_window_size=0, max_faces_from_image=0, final_output_path=None, device_config=None, workers_per_gpu=1):
        if type == 'landmarks-manual':
            for x in input_data:
                x.manual = True
        ...
        self.devices = ExtractSubprocessor.get_devices_for_config(self.type, device_config, workers_per_gpu)
...
def main(detector=None,
    ...
    if images_found != 0:
        if detector == 'manual':
            ...
            data = ExtractSubprocessor ([ ExtractSubprocessor.Data(Path(filename)) for filename in input_image_paths ], 'landmarks-manual', image_size, jpeg_quality, face_type, output_debug_path if output_debug else None, manual_window_size=manual_window_size, device_config=device_config, workers_per_gpu=1).run()
            ...
            data = ExtractSubprocessor ([ ExtractSubprocessor.Data(Path(filename)) for filename in input_image_paths ],
                                         ...
                                         workers_per_gpu=workers_per_gpu).run()
