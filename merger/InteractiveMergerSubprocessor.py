...
from core.imagelib.stco_color import stco_correct
...
class InteractiveMergerSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.stco_enabled = client_dict.get('stco_enabled', False)
            self.stco_strength = client_dict.get('stco_strength', 0.8)
        ...
        def process_data(self, pf): #pf=ProcessingFrame
            ...
            if len(frame_info.landmarks_list) == 0:
                ...
                # Apply STCO skin color correction if enabled
                if self.stco_enabled:
                    img_bgr = final_img[...,0:3]
                    target_img = cv2_imread(filepath)
                    if target_img is not None:
                        img_bgr = stco_correct(img_bgr, target_img, strength=self.stco_strength)
                    final_img = np.concatenate([img_bgr, final_img[...,3:4]], axis=-1)
    ...
    #override
    def __init__(self, is_interactive, merger_session_filepath, predictor_func, predictor_input_shape, face_enhancer_func, xseg_256_extract_func, merger_config, frames, frames_root_path, output_path, output_mask_path, model_iter, subprocess_count=4, stco_enabled=False, stco_strength=0.8):
        if len (frames) == 0:
        ...
        self.stco_enabled = stco_enabled
        self.stco_strength = stco_strength
    ...
    def process_info_generator(self):
        ...
        for i in r:
            yield 'CPU%d' % (i), {}, {'device_idx': i,
                                      ...
                                      'stco_enabled': self.stco_enabled,
                                      'stco_strength': self.stco_strength
                                      }
