...
class ModelBase(object):
    def __init__(self, is_training=False,
        ...
        if self.model_data_path.exists():
            ...
            if self.iter != 0:
                ...
                raw_sample = model_data.get('sample_for_preview', None)

                if raw_sample is not None:
                    try:
                        if isinstance(raw_sample, list):
                            decompressed = []
                            for sample_group in raw_sample:
                                if isinstance(sample_group, list):
                                    decompressed_group = []
                                    for item in sample_group:
                                        if hasattr(item, 'shape') and len(item.shape) == 3 and item.dtype == np.uint8:
                                            import cv2
                                            img = cv2.imdecode(item, cv2.IMREAD_COLOR)
                                            if img is not None:
                                                img = img.astype(np.float32) / 255.0
                                            decompressed_group.append(img)
                                        else:
                                            decompressed_group.append(item)
                                    decompressed.append(decompressed_group)
                                else:
                                    decompressed.append(sample_group)
                            self.sample_for_preview = decompressed
                        else:
                            self.sample_for_preview = raw_sample
                    except Exception as e:
                        io.log_info(f"Preview decompression warning: {e}")
                        self.sample_for_preview = raw_sample
                else:
                    self.sample_for_preview = None
    ...
    def save(self):
        ...
        if self.sample_for_preview is not None:
            try:
                if isinstance(self.sample_for_preview, list):
                    sample_for_preview_save = []
                    for sample_group in self.sample_for_preview:
                        if isinstance(sample_group, list):
                            compressed_group = []
                            for s in sample_group[:2]:
                                if hasattr(s, 'shape') and len(s.shape) == 3:
                                    if self.compress_preview:
                                        import cv2
                                        encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 85]
                                        _, buf = cv2.imencode('.jpg', (s * 255).astype(np.uint8), encode_param)
                                        compressed_group.append(buf)
                                    else:
                                        compressed_group.append(s.copy())
                                else:
                                    compressed_group.append(s)
                            sample_for_preview_save.append(compressed_group)
                        else:
                            sample_for_preview_save.append(sample_group)
                else:
                    sample_for_preview_save = self.sample_for_preview
            except Exception as e:
                io.log_info(f"Preview compression warning: {e}")
                sample_for_preview_save = None

        model_data = {
            'iter': self.iter,
            'options': self.options,
            'loss_history': self.loss_history[-50000:] if len(self.loss_history) > 50000 else self.loss_history,
            'sample_for_preview' : sample_for_preview_save,
    ...
    def train_one_iter(self):
        ...
        if len(self.loss_history) > 100000:
            keep_ratio = max(1, len(self.loss_history) // 50000)
    ...
    def process(self, sq):
        while True:
            try:
                ...
                if isinstance(e, EOFError) or (hasattr(e, 'args') and 'Queue' in str(e)):
                    break
                time.sleep(0.01)
