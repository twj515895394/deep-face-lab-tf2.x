...
class SampleLoader:
    ...
    def clear_cache():
        if len(SampleLoader.samples_cache) > 0:
            ...
            SampleLoader.samples_cache.clear()
            import gc
            gc.collect()
            return cleared
        return 0
    ...
    def _manage_cache_size():
        while len(SampleLoader.samples_cache) > SampleLoader._max_cache_size:
            oldest_key = next(iter(SampleLoader.samples_cache))
            del SampleLoader.samples_cache[oldest_key]
    @staticmethod
