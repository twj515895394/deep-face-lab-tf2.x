...
class SampleProcessor(object):
    ...
    @staticmethod
    def process (samples, sample_process_options, output_sample_types, debug, ct_sample=None, cached_bgr=None, cached_ct_bgr=None):
        ...
        for sample in samples:
            ...
            if cached_bgr is not None:
                sample_bgr = cached_bgr
            else:
                sample_bgr = sample.load_bgr()
            sample_landmarks = sample.landmarks
            ct_sample_bgr = cached_ct_bgr if cached_ct_bgr is not None else None
            h,w,c = sample_bgr.shape
            ci_w, ci_h = int(w), int(h)
            
            def get_full_face_mask():   
                xseg_mask = sample.get_xseg_mask()                                     
                if xseg_mask is not None:           
                    if xseg_mask.shape[0] != h or xseg_mask.shape[1] != w:
                        xseg_mask = cv2.resize(xseg_mask, (ci_w,ci_h), interpolation=cv2.INTER_CUBIC)                    
            ...
            for opts in output_sample_types:
                resolution     = int(opts.get('resolution', 0))
                ...
                if sample_type == SPST.FACE_IMAGE or sample_type == SPST.FACE_MASK:
                    ...
                    if sample_type == SPST.FACE_MASK: 
                        ...
                        if random_hsv_shift_amount != 0:
                            ...
                            img_h = (img_hsv[...,0:1].astype(np.float32) + float(rnd_state.randint(-h_amount, h_amount+1)) ) % 180.0
                            img_s = np.clip( img_hsv[...,1:2].astype(np.float32) + (float(rnd_state.random())-0.5)*a, 0, 1 )
                            img_v = np.clip( img_hsv[...,2:3].astype(np.float32) + (float(rnd_state.random())-0.5)*a, 0, 1 )
